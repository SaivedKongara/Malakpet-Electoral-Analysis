...
This project extracts, structures, and analyzes voter and election-result data at the polling-booth level to support political strategy and campaign decision-making. It combines an automated document-extraction pipeline with an Excel-based analytical framework (seat grading, swing-voter classification and margin analysis) and an in-progress interactive Power BI dashboard.
...
```
## 4. Data Extraction: Challenges and Engineering Decisions
...
Demographic–vote correlation (Power BI dashboard in progress) — Booth/area-level demographic composition (from the extracted voter data) is correlated against party vote share (CORREL/SLOPE) to identify areas of demographic concentration associated with particular parties' vote strength. This is an ecological-level association, not an individual-level claim — see Limitations.
...
│   └── booth_files/            # Full Form 20 scans, all 3 years — public data, no redaction needed
...
├── 04_working_review/          # Malakpet_2014/2018/2023_pdf_to_xlsx.xlsx (raw PDF→Excel per year)
│                                # + Cleaned_Raw_Output.xlsx (flags fixed, not yet fully cleaned)
├── 05_final_analysis/          # Cleaned master + final analysis sheets (Area Analysis, Seat Grading, Swing Voter)
├── 06_dashboard/               # [planned] Power BI roadmap and screenshots — not yet in this repo
├── assets/                     # [planned] Diagrams, screenshots — not yet in this repo
├── requirements.txt            # Python dependencies for 02_pipeline
└── README.md
```
...
pdf2image also requires the poppler-utils system package (apt-get install poppler-utils on Linux, brew install poppler on macOS). The extraction notebook expects a Gemini API key available as an environment variable, loaded via python-dotenv from a local .env file (not committed).
...
Python · Gemini Vision API · Excel · Power BI · DAX
